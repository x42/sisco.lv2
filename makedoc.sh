...
<dt>(4) Channel Settings</dt><dd>Allows to alter the waveform display. Vertical Zoom and polarity can be set with the "Amp" dial in a range from -6 to +6 (linear scale, corresponding to 30dB). The 'gang' button enables identical amplitude scaling for all channels. X and Y offset are in percent of the screen width. These settings apply to the display-buffer only and can also be modified even when acquisition is paused. A Channel can be hidden (display, green button) and the display can be frozen (memory, red button).</dd>
